...
| S19 | **L** (was filed HIGH — **corrected by runtime test**) | The `scope_allows_write` check in `_mcp_token_state` is gated on `if on_api`, so it is skipped for `on_mount` requests, and every MCP `tools/call` is a POST — which *looks* like an `mcp:read` connect token could invoke platform write tools on `/mcp/platform`. **It cannot**, and I only learned that by actually running the tool: the platform tools re-enter internally at `/api/v1/platform-read/*` and `/api/v1/platform-tools/*`, and **neither prefix is in `MCP_TOKEN_PATH_PREFIXES`**, so the osmcp_ token is rejected (401) on the internal call regardless of scope. The path allowlist enforces the boundary the scope check fails to. Real residual: the skipped scope check is a latent defect that becomes live the moment anyone adds a platform path to the allowlist — fix it, but it is not an exploit today. **Lesson: this was a false-positive HIGH that survived static reasoning and died on first execution — exactly the verification pass OpenSweep isn't running.** | `app.py:174-190`, `MCP_TOKEN_PATH_PREFIXES app.py:144-158` |
...
| G12 | **No external interactive client can write to OpenSweep via MCP — confirmed three ways.** (a) Shared `OPENSWEEP_AUTH_TOKEN` → `get_local_user()` → hardcoded `LOCAL_ORG_UID`, which owns no repositories → every repo-scoped platform tool 404s (no platform-admin bypass in `require_repo_in_org`). (b) `opensweep connect` osmcp_ token → 401 on the platform mount's internal routes (`/api/v1/platform-*` not in `MCP_TOKEN_PATH_PREFIXES`). (c) The external `/mcp` mount is read-only by design (only `comment_create`) and disabled by default. **The only working write path is a per-run `osrt_` token bound to an active run** — i.e. the discovery loop. This is why the platform surface `mcp_app.py` calls "no consumer yet" has, in fact, no usable external consumer. `LOCAL_ORG_UID` is a constant, not configurable, so there is no zero-code workaround | `dependencies.py:56`, `local_user.py:13-22`, `tenancy.py:35-39`, `app.py:144-158`, `mcp_app.py:62-74` |
