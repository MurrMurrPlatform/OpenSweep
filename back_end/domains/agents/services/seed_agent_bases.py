...
_AGENT_BASES: dict[str, dict] = {
    ...
    "map-areas": {
        ...
        "body": (
            ...
            "tool.\n"
            "\n"
            "# The two partitions (the invariant this run exists to satisfy)\n"
            "\n"
            "The map has TWO independent axes, and each one must partition the\n"
            "repository on its own: every tracked file belongs to exactly one LEAF\n"
            "on the subsystem axis (kinds `subsystem` and `ignore`) AND to exactly\n"
            "one LEAF on the feature axis (kinds `feature` and `feature_ignore`).\n"
            "Finishing one axis says nothing about the other — a complete subsystem\n"
            "partition beside a handful of feature areas covering one directory is\n"
            "a HALF-DONE map, not a done one. Paths that belong to no real area on\n"
            "an axis still have to land somewhere: use `ignore` on the subsystem\n"
            "axis and `feature_ignore` on the feature axis, each with its reason in\n"
            "the spec. Do NOT call complete_run until both partitions are complete.\n"
            "\n"
            "# Subsystem areas\n"
            "\n"
            ...
            "hole. `ignore` covers the SUBSYSTEM axis only; the same files (and\n"
            "everything else no product flow owns — config, tooling, static assets)\n"
            "need `feature_ignore` areas to close the FEATURE axis. Both kinds take\n"
            "the same discipline: a reason per area, never one silent catch-all.\n"
            "\n"
            "# Feature areas\n"
            "\n"
            ...
            "The same list reports how much of the axis you just proposed into is\n"
            "still unclaimed — treat a remaining-files warning exactly like an\n"
            "overlap: keep proposing until it disappears.\n"
            "Do not finish with unresolved warnings, and do not call complete_run\n"
            "while either axis still has unclaimed files — a run that does is sent\n"
            "back to finish the partition."
        ),
    },
}
