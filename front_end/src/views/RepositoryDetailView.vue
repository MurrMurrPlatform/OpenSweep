...
/**
 * Workspace home — the page onboarding lands on.
 *
 * Its job is to answer "what do I do here?" for a brand-new user (one obvious
 * way to start a sweep) and "what happened since I last looked?" for everyone
 * else (open findings, PRs waiting on a human, recent runs). The at-a-glance
 * numbers come from existing endpoints; the run strip refreshes itself off the
 * shared /runs/active heartbeat instead of a Refresh button.
 */
...
// In-flight runs anywhere in this repo — surfaced as a chip in the header, and
// used as the dashboard's liveness heartbeat (see the signature watcher below).
      ...
      <template v-else>
        ...
        <Card>
          <CardHeader class="flex-row flex-wrap items-center justify-between gap-2 space-y-0 p-4">
            ...
            <Button
              as="router-link"
              :to="{ name: 'runs', params: { repoSlug: repo.slug } }"
              variant="ghost"
              size="sm"
            >
              All runs
